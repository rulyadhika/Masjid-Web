    ...
    <div class="col-12">
        <div class="card card-success card-outline">
            <div class="d-flex justify-content-between py-2 px-3 border-bottom">
                ...
                <button class="btn btn-sm btn-success tambah-data-btn" data-toggle="modal" data-target="#bannerModal">Tambah
                    Data</button>
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        <?php foreach ($data as $d) : ?>
                            <tr>
                                <td><?= $no++; ?></td>
                                <td>
                                    <img style="max-height: 100px;object-fit:cover;width:100%;" src="<?= base_url("asset/images/banner/" . $d['gambar']); ?>" alt="" class="img-fluid">
                                </td>
                                <td style="vertical-align: middle;"><?= $d['judul']; ?></td>
...
<!-- Modal -->
<div class="modal fade" id="bannerModal" data-backdrop="static" data-keyboard="false" aria-labelledby="bannerModalLabel" aria-hidden="true">
    <form action="<?= base_url("admin/banner/insertBanner"); ?>" method="POST" id="bannerForm" enctype="multipart/form-data">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="bannerModalLabel">Tambah data banner</h5>
                ...
                <div class="modal-body">
                    <div class=" mb-2">
                        ...
                        <img class="img-fluid img-preview d-block mx-auto" src="" alt="Silahkan upload gambar terlebih dahulu">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group mb-1">
                            <div class="custom-file">
                                <input type="file" class="custom-file-input <?= isset($errors['gambar']) ? 'is-invalid' : ''; ?>" id="gambar" name="gambar" aria-describedby="inputGroupFileAddon01" onchange="previewImg()">
...
<script>
    $(".tambah-data-btn").on("click", () => {
        ...
        $("#bannerForm").attr("action", "<?= base_url('admin/banner/insertBanner') ?>");
        console.log($("#bannerForm").attr("action"));
    });

    $(".edit-btn").on("click", function() {
        $('#bannerModal').modal('show');
        $('.modal-title').text('Ubah data banner');
        $("input[name=id]").val(this.dataset.id);

        $("#bannerForm").attr("action", "<?= base_url('admin/banner/updateBanner') ?>");

        $.ajax({
            type: "POST",
            url: "<?= base_url('admin/banner/getsingledata') ?>",
            data: {
                id: this.dataset.id
            },
            headers: {
            ...
            success: function(data) {
                ...
                $(".img-preview").attr("src",
                    "<?= base_url('asset/images/banner') ?>/" + data.gambar);
            },
        });
    })
